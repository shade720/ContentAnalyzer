...
using DataCollectionService.BusinessLogicLayer.SocialNetworkClients.VkClient.ScannerManager;
using Serilog;
    ...
    private async Task StartScanAsync()
    {
        try
        {
            ...
            while (!StopScanToken.Token.IsCancellationRequested)
            {
                ...
                await Task.Delay(int.Parse(Configuration["ScanPostDelay"]), StopScanToken.Token).ContinueWith(_ => { }); //to avoid exception
